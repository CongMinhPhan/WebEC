...
const AdminDeleteProduct = ({ 
  productData, // Dữ liệu của sản phẩm
  onClose, // Hàm được gọi khi muốn đóng modal
  fetchdata // Hàm được gọi sau khi xóa để cập nhật
}) => {

  const handleDelete = async (id) => { // truyen id
    const response = await fetch(SummaryApi.deleteProduct.url,{ // goi API yeu cau xoa
